...
def _ok(msg):
    return f"{C.GREEN}[OK]  {msg}{C.RESET}"
...
def check_python_version(report: HealthReport):
    ...
    if v >= (3, 9):
        report.add(
            CheckResult(name, STATUS_OK, f"Python {v.major}.{v.minor}.{v.micro}")
        )
    elif v >= (3, 7):
        report.add(
            CheckResult(
                name, STATUS_WARN, f"Python {v.major}.{v.minor} — 建议升级到 3.9+"
            )
        )
    else:
        report.add(
            CheckResult(
                name, STATUS_FAIL, f"Python {v.major}.{v.minor} — 版本过低，可能不兼容"
            )
        )
...
def check_src_modules(report: HealthReport):
    ...
    for mod_path, required, symbols in MODULE_CHECKS:
        ...
        if mod is None:
            ...
            report.add(
                CheckResult(f"模块:{short}", level, f"导入失败", detail=err or "")
            )
        else:
            # 检查必需符号
...
def check_critical_interface(report: HealthReport, auto_fix: bool):
    ...
    if not evo_path.exists():
        report.add(
            CheckResult(
                "接口:run_evolution.py", STATUS_WARN, "文件不存在，跳过接口检测"
            )
        )
        return
    ...
    if (
        "MarketDataLoader" in content
        and "DataLoader as MarketDataLoader" not in content
    ):
        issues.append(
            "run_evolution.py 使用了不存在的类 MarketDataLoader（应为 DataLoader）"
        )
        if auto_fix:
    ...
    if issues and not fixes_applied:
        ...
        report.add(
            CheckResult(
                "接口:MarketDataLoader", STATUS_OK, "MarketDataLoader 导入写法正常"
            )
        )
...
def check_data_files(report: HealthReport):
    ...
    for rel_path, (label, preferred) in files.items():
        ...
        if full.exists():
            ...
            report.add(
                CheckResult(
                    f"数据:{label}", STATUS_OK, f"{full.name} ({size_mb:.1f} MB)"
                )
            )
            any_data = True
        else:
            level = STATUS_WARN if not preferred else STATUS_WARN
            report.add(CheckResult(f"数据:{label}", STATUS_WARN, f"不存在: {rel_path}"))
    ...
    if not any_data:
        ...
        report.add(CheckResult("数据:总体", STATUS_OK, f"至少存在部分数据文件"))
...
def check_e2e_import_chain(report: HealthReport):
    ...
    if all_ok:
        report.add(
            CheckResult(
                "链路:end-to-end", STATUS_OK, "run_evolution.py 核心导入链路全部连通"
            )
        )
    else:
...
def check_output_dirs(report: HealthReport):
    ...
    for d in dirs:
        ...
        try:
            ...
            report.add(CheckResult(f"目录:{d}", STATUS_WARN, f"无法创建或写入: {e}"))
...
def print_report(report: HealthReport):
    ...
    if report.passed:
        ...
        print(
            f"  {C.YELLOW}Tip: run  python health_check.py --fix  to auto-fix known issues{C.RESET}"
        )
    print(f"{C.BOLD}{C.WHITE}{'=' * 55}{C.RESET}\n")
