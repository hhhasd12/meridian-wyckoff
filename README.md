...
│                                                               │
├───────────────────────────────────────────────────────────────┤
│  兼容层 src/core/（已废弃，保留向后兼容 shim 导入）          │
...
│   └── TECH_SPECS.md                       技术规范
├── plans/                                  架构设计方案
├── run_live.py                             ★ 生产启动脚本（调用 WyckoffApp）
...
> **架构说明**：v2.0 采用三层插件化架构。`src/kernel/` 为不可插拔的内核层，
> `src/plugins/` 为可插拔的业务插件层（每个插件包含 `plugin-manifest.yaml` 和 `plugin.py`），
> `src/core/` 为已废弃的兼容层（仅保留 shim 导入，实际逻辑在 `*_legacy.py` 中）。
...
# 预期结果
# 856 passed
# 内核层 86 + 插件层 454 + 兼容层 306 + 入口 10 = 856
```
...
### 第二阶段（已完成 ✅）
- [x] ~~合并 `run_live.py` 与 `run_system.py` 为单一生产入口~~ — `run_live.py` 已是唯一生产入口
- [x] ~~解决 `DataPipeline` 同名类冲突~~ — 重命名为 `DataFlowPipeline`
- [x] ~~清理 `scripts/` 目录重复数据获取脚本~~ — 删除6个冗余脚本

### 第三阶段（已完成 ✅ — v2.0 插件化重构）
- [x] ~~三层插件化架构重构~~ — 内核层 + 插件层 + 兼容层
- [x] ~~13个业务插件迁移~~ — 全部插件包含 manifest + plugin.py
...
| `plans/plugin_architecture_plan.md` | 插件化架构设计方案 |
