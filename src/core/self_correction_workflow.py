...
try:
    ...
    from .weight_variator import MutationType, WeightVariator
    from .wfa_backtester import PerformanceMetric, ValidationResult, WFABacktester
except ImportError:
    # 备用导入
    from mistake_book import ErrorPattern, ErrorSeverity, MistakeBook, MistakeType
    from weight_variator import WeightVariator
    from wfa_backtester import ValidationResult, WFABacktester
        ...
        try:
            ...
            if total_errors < self.min_errors_for_correction:
                return CorrectionResult(
                    ...
                    error_message=f"错误数量不足: {total_errors} < {self.min_errors_for_correction}",
        ...
        try:
            ...
            if not weight_adjustments:
                ...
                result_details = {
                    ...
                    "mutation_details": [{"_config_full": c, "config_summary": self._summarize_config(c)} for c in mutated_configs],
                    "pattern_based_mutations": 0,
                    "random_mutations": len(mutated_configs),
                    "source_patterns": [],
                }
                metrics = {
                    "mutation_diversity": len(mutated_configs) / self.max_mutations_per_cycle,
            ...
            for i, adjustment in enumerate(
                ...
                try:
                    ...
                    # 获取种群中的变异配置（跳过index 0，那是未变异的base config）
                    if (
                        hasattr(self.weight_variator, "population")
                        and len(self.weight_variator.population) > 1
                    ):
                        # 从index 1开始取，这些才是真正的变异体
                        pop_idx = min(i + 1, len(self.weight_variator.population) - 1)
                        mutated_config = [self.weight_variator.population[pop_idx]["config"]]
                    else:
                        # 回退到简单变异
                        mutated_config = [
    ...
    def _evaluate_correction(self) -> CorrectionResult:
        ...
        try:
            ...
            metrics = {
                ...
                "system_stability": 0.8,  # 模拟值
                "performance_trend": 0.1,  # 模拟值
            }
