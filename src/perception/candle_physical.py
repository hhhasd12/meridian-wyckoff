...
def create_candle_from_series(series: dict[str, float]) -> CandlePhysical:
    ...
    return CandlePhysical(
        open=series.get("open", 0),
        high=series.get("high", 0),
        low=series.get("low", 0),
        close=series.get("close", 0),
...
if __name__ == "__main__":
    ...
    # 测试示例3：大实体
    candle3 = CandlePhysical(open=100, high=102, low=98, close=108, volume=2000)
