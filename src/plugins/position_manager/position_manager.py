...
class PositionManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.signal_exit_logic = SignalExitLogic(
            config.get("signal_exit", {})
        )
    ...
    def close_position(
        ...
        if partial_ratio and 0 < partial_ratio < 1:
            close_size = position.size * partial_ratio
    ...
    def force_close_all(self, exit_prices: Dict[str, float]) -> List[TradeResult]:
        ...
        for symbol, position in list(self.positions.items()):
            exit_price = exit_prices.get(symbol, position.entry_price)
