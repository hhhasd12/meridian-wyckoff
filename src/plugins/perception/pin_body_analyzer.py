...
def analyze_pin_vs_body(
    ...
    if isinstance(context, dict):
        ...
        if "market_regime" in context_dict:
            ...
            if not isinstance(regime_value, MarketRegimeType):
                try:
                    context_dict["market_regime"] = MarketRegimeType(
                        str(regime_value).upper()
                    )
    ...
    # 注意：努力结果分析的置信度加成已在步骤7中计算，此处不再重复累加
...
def get_recommendation(
    ...
    candle: Optional[CandlePhysical] = None,
) -> dict[str, Any]:
    """
    根据分析结果生成交易建议
    ...
    if result.is_pin_dominant:
        ...
        # 实体主导表示趋势延续
        if candle is not None and _is_at_support_resistance(candle, context):
            # 在关键位置的实体主导
