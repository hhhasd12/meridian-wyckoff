...
此模块从 src/core/market_regime.py 迁移而来，保持原有逻辑不变。
MarketRegime 枚举和 RegimeDetector 类可被其他插件直接导入使用。
"""
...
class RegimeDetector:
    ...
    def detect_regime(self, df: pd.DataFrame) -> Dict:
        ...
        self.regime_history.append(
            (df.index[-1], regime, confidence)  # type: ignore[arg-type]
        )
    ...
    def _judge_regime(self, metrics: Dict) -> Tuple[MarketRegime, float, List[str]]:
        ...
        if metrics["atr_ratio"] > self.volatility_threshold:
            ...
            reasons.append(
                ...
                f">={self.volatility_threshold}，波动剧烈"
            )
        ...
        best_regime = max(
            scores,
            key=lambda k: scores.get(k, 0),  # type: ignore[arg-type]
        )
