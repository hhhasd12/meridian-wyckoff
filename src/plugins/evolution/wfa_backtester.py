...
class WFABacktester:
    ...
    def _analyze_windows(self, windows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # ---- P1-1 修复：将 COMPOSITE_SCORE 写入 avg_performance ----
        avg_performance["COMPOSITE_SCORE"] = composite_score
    ...
    def _simulate_performance(
        ...
        # ---- P1-2 修复：使用 config 哈希 + 窗口索引作为确定性种子 ----
        # 构建 config 的确定性哈希
        config_hash = hashlib.md5(str(sorted(leaf_values)).encode()).hexdigest()
        config_seed = int(config_hash[:8], 16)

        # 如果有 data 参数，用其长度作为窗口索引的近似
        window_idx = len(data) if data is not None else 0
        deterministic_seed = config_seed + window_idx

        # 使用独立的 Random 实例，避免影响全局随机状态
        rng = random.Random(deterministic_seed)
        noise = rng.uniform(-0.05, 0.05)
        base_perf = max(0.1, min(0.9, base_perf + noise))

        # 稳定性噪声也使用确定性种子
        stability_noise = rng.uniform(-0.05, 0.05)
        ...
        return {
            ...
            PerformanceMetric.STABILITY_SCORE.value: (
                0.6 + base_perf * 0.3 + stability_noise
            ),
