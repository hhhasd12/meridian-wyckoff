...
class BacktestValidatorAgent(BaseAgent):
    ...
    def _handle_request(self, message: AgentMessage) -> AgentMessage:
        ...
        if request_type == "get_status":
            ...
            return message.create_response(
                {
                    "backtests": [
                        self._backtest_to_dict(b) for b in self.backtest_history
                    ]
                }
            )
    ...
    def _run_backtest(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data is None:
            return {
                "error": f"无法加载数据: {data_source}，可用数据: {list(AVAILABLE_DATA.keys())}"
            }

        self.logger.info(
            f"加载数据成功: {len(data)} 条K线, 时间范围: {data.index[0]} ~ {data.index[-1]}"
        )
    ...
    def _generate_signals_from_state_machine(
