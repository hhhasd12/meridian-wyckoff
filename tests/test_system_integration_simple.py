...
# 计算移动平均
if len(df_test) >= 20:
    sma_20 = df_test["close"].rolling(20).mean().tail(1).values[0]
    current_close = df_test["close"].tail(1).values[0]
    print(f"  20周期SMA: {sma_20:.2f}")
    print(f"  当前价格 vs SMA20: {(current_close - sma_20):+.2f}")
print()
...
if len(df_test) >= 50:
    # 使用values避免类型错误
    close_values = df_test["close"].values
    ma50_values = pd.Series(close_values).rolling(50).mean().values
    ma20_values = pd.Series(close_values).rolling(20).mean().values

    if len(ma50_values) > 0 and len(ma20_values) > 0:
        ma50 = ma50_values[-1]
        ma20 = ma20_values[-1]
...
# 检查趋势是否已定义
if len(df_test) >= 50:
    ma50 = df_test["close"].rolling(50).mean().iloc[-1]
    ma20 = df_test["close"].rolling(20).mean().iloc[-1]
...
# 规则4: 威科夫模式
if "吸筹" in locals().get("wyckoff_phase", ""):
    reasoning.append("威科夫吸筹模式")
    confidence += 0.2
elif "派发" in locals().get("wyckoff_phase", ""):
...
if missing_modules:
    print(f"  ⚠️  缺失 {len(missing_modules)} 个核心模块:")
...
if missing_tests:
    print(f"  ⚠️  缺失 {len(missing_tests)} 个测试文件")
else:
    print("  ✅ 所有测试文件存在")
print()
