...
def test_mutation_operator_base_class():
    ...
    operator = ThresholdMutationOperator(
        target_module="test_module", parameters=["test_param"], max_change=0.05
    )
...
def test_threshold_mutation_operator():
    ...
    test_cases = [
        ...
        (
            0.7,
            ErrorPattern.FREQUENT_FALSE_NEGATIVE,
            0.3,
            ErrorSeverity.MEDIUM,
            "假阴性",
        ),
    ...
    for current_value, pattern, frequency, severity, description in test_cases:
        ...
        print(
            f"{description}: {current_value:.3f} -> {new_value:.3f} (变化: {change_percent:.1f}%)"
        )

        # 验证变化幅度不超过最大变化
        assert change_percent <= 5.0, (
            f"{description}变化幅度超过5%: {change_percent:.1f}%"
        )
...
def test_weight_mutation_operator():
    ...
    # 测试权重
    weights = {"W": 0.30, "D": 0.25, "H4": 0.20, "H1": 0.15, "M15": 0.07, "M5": 0.03}
    ...
    for pattern, description in test_patterns:
        new_weights = operator.mutate(
            value=weights, pattern=pattern, frequency=0.5, severity=ErrorSeverity.MEDIUM
        )
        ...
        for key in weights:
            if key in new_weights:
                ...
                print(
                    f"  {key}: {weights[key]:.3f} -> {new_weights[key]:.3f} (变化: {change:.1f}%)"
                )
...
def test_error_handling():
    ...
    try:
        operator.mutate(
            "invalid", ErrorPattern.FREQUENT_FALSE_POSITIVE, 0.5, ErrorSeverity.MEDIUM
        )
    ...
    weight_operator = WeightMutationOperator(
        target_module="test_module", parameters=["param1", "param2"], max_change=0.05
    )

    try:
        weight_operator.mutate(
            "invalid", ErrorPattern.FREQUENT_FALSE_POSITIVE, 0.5, ErrorSeverity.MEDIUM
        )
        assert False, "应抛出TypeError"
    except TypeError as e:
